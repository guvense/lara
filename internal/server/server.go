...
var (
	corsMethods        = []string{"GET", "HEAD", "POST", "PUT", "OPTIONS", "DELETE", "PATCH", "TRACE", "CONNECT"}
	corsHeader        = []string{"X-Requested-With", "Content-Type", "Authorization"}
	corsExposedHeaders = []string{"Cache-Control", "Content-Language", "Content-Type", "Expires", "Last-Modified", "Pragma"}
)

func PrepareAccessControl(configCors ConfigCORS) (h []handlers.CORSOption) {
	h = append(h, handlers.AllowedMethods(corsMethods))
	h = append(h, handlers.AllowedHeaders(corsHeader))
	h = append(h, handlers.ExposedHeaders(corsExposedHeaders))
	return
}
